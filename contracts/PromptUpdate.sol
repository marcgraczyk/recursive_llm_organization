...
contract PromptUpdate {
    ...
    IUniswapV3Factory private constant uniswapV3Factory = IUniswapV3Factory(UNISWAP_V3_FACTORY_ADDRESS);
    ...
    constructor(address _usdcTokenAddress, address _uniswapRouterAddress, address _daoTokenAddress, address _myTokenAddress) {
    ...
    function updatePrompt(string memory newPrompt, uint256 usdcAmount) public {
        ...
        uint256 totalSupply = uniswapPool.totalSupply(); // Assuming this is a proxy for liquidity
        ...
        require(usdcAmount > requiredBid, "Bid does not meet the minimum requirement");
        usdcToken.safeTransferFrom(msg.sender, address(this), usdcAmount);

        // reward mechanism

        if (lastMarketCap != 0 && lastPrompter != address(0)) {
                uint256 marketCapChange = currentMarketCap > lastMarketCap ? currentMarketCap - lastMarketCap : 0;
    ...
    function calculateEquivalentDAOTokenAmount(uint256 usdcAmount) private view returns (uint256) {
       
        address poolAddress = IUniswapV2Factory(uniswapRouter.factory()).getPair(address(usdcToken), daoTokenAddress);
        
        // Ensure the pool exists
        require(poolAddress != address(0), "Pool does not exist");

        // Fetch the current reserves from the pool
        (uint reserveUSDC, uint reserveDAO,) = IUniswapV2Pair(poolAddress).getReserves();

        // Determine the correct order of reserves
        // This assumes USDC and your DAO token follow a specific order; adjust as needed
        (reserveUSDC, reserveDAO) = address(usdcToken) < daoTokenAddress ? (reserveUSDC, reserveDAO) : (reserveDAO, reserveUSDC);

        // Calculate how much DAO tokens are needed based on current pool rate
        uint256 daoTokenAmount = usdcAmount * reserveDAO / reserveUSDC;

        return daoTokenAmount;

    }

    function getCurrentPrice(uint24 fee) public view returns (uint256 price) {

        address poolAddress = uniswapV3Factory.getPool(address(usdcToken), daoTokenAddress, fee);
        require(poolAddress != address(0), "Pool does not exist");
    ...
    function approximateTickFromPrice(uint256 price) public pure returns (int24 tick) {
